/**
 * Funding Rate Spike Trading Strategy v3
 *
 * FR V2 with BTC EMA200 regime filter — blocks entries during bear markets while
 * maximizing bull retention. All other logic is identical to v2.
 *
 * Key addition over v2:
 * - BTC EMA200 regime filter: skips ALL new entries when BTC/USDT daily price
 *   is below its 200-period EMA (bear market). Existing positions are managed
 *   normally (stops, TP, trailing stop, time exit still fire).
 * - BTC daily candles must be injected externally:
 *     (strategy as any)._btcDailyCandles = [{ timestamp, close }, ...]
 *
...
function calculateEMAValue(values: number[], period: number): number {
...
interface StrategyState {
  ...
  _entryATR: number;
  /** BTC daily candles injected externally for regime filter */
  _btcDailyCandles: Array<{ timestamp: number; close: number }>;
  /** Timestamp of last regime check (ms) */
  _lastRegimeCheck: number;
  /** Cached regime state: true = bull (BTC above SMA200), false = bear */
  _isBullRegime: boolean;
}
...
const strategy: Strategy = {
  name: 'funding-rate-spike-v3',
  description:
    'FR V2 with BTC EMA200 regime filter — blocks entries during bear markets while maximizing bull retention',
  version: '3.0.0',
  ...
  params: [
    ...
    {
      name: 'useRegimeFilter',
      type: 'boolean',
      default: true,
      description: 'Block new entries when BTC is below its daily SMA200 (bear market filter)',
    },
    {
      name: 'regimeSMAPeriod',
      type: 'number',
      default: 200,
      min: 50,
      max: 300,
      step: 50,
      description: 'SMA period on BTC daily candles for regime determination',
    },
    {
      name: 'bearMode',
      type: 'select',
      default: 'block',
      options: ['block', 'shortOnly', 'mirror'],
      description: 'Bear regime behavior: block all entries, allow only shorts, or mirror signals (invert long↔short)',
    },
    {
      name: 'regimeMAType',
      type: 'select',
      default: 'ema',
      options: ['sma', 'ema'],
      description: 'Moving average type for BTC regime filter: SMA or EMA (EMA reacts faster to trend changes)',
    },
  ] as StrategyParam[],
  ...
  init(context: StrategyContext): void {
    ...
    self._entryATR = 0;
    // Preserve _btcDailyCandles if injected before init (external injection pattern)
    self._btcDailyCandles = self._btcDailyCandles || [];
    self._lastRegimeCheck = 0;
    self._isBullRegime = true; // Default to bull if no BTC data available
    context.log('Initialized funding-rate-spike-v3');
  },
  ...
  onBar(context: StrategyContext): void {
    ...
    const useRegimeFilter = params.useRegimeFilter as boolean;
    const regimeSMAPeriod = params.regimeSMAPeriod as number;
    const bearMode = params.bearMode as string;
    const regimeMAType = params.regimeMAType as string;
    ...
    // --- REGIME FILTER CHECK ---
    // Recalculate at most once per day to save computation
    if (useRegimeFilter && self._btcDailyCandles && self._btcDailyCandles.length > 0) {
      const dayMs = 24 * 60 * 60 * 1000;
      if (currentCandle.timestamp - self._lastRegimeCheck >= dayMs || self._lastRegimeCheck === 0) {
        self._lastRegimeCheck = currentCandle.timestamp;

        // Get BTC daily closes up to (and including) current timestamp
        const btcCloses = self._btcDailyCandles
          .filter(c => c.timestamp <= currentCandle.timestamp)
          .map(c => c.close);

        if (btcCloses.length >= regimeSMAPeriod) {
          const btcPrice = btcCloses[btcCloses.length - 1];
          let btcMA: number;

          if (regimeMAType === 'ema') {
            btcMA = calculateEMAValue(btcCloses.slice(-regimeSMAPeriod * 2), regimeSMAPeriod);
          } else {
            const smaSlice = btcCloses.slice(-regimeSMAPeriod);
            btcMA = smaSlice.reduce((s, v) => s + v, 0) / smaSlice.length;
          }

          self._isBullRegime = btcPrice > btcMA;

          // Emit indicators for chart overlay
          context.setIndicator('btcMA', btcMA);
          context.setIndicator('btcPrice', btcPrice);
          context.setIndicator('regimeBull', self._isBullRegime ? 1 : 0);
        }
      }
    }
    ...
    // NOTE: Regime filter does NOT affect existing position management.
    //       Stops, TP, trailing stop, and time-based exits always fire.
    // =========================================================================
    ...
    // Regime filter: handle bear regime based on bearMode
    const inBearRegime = useRegimeFilter && !self._isBullRegime;
    if (inBearRegime && bearMode === 'block') {
      return; // Block ALL new entries in bear market
    }
    ...
    // Bear regime signal modifications (applied before trend filter so filter logic is coherent)
    if (inBearRegime && bearMode === 'mirror') {
      // Invert signals: long→short, short→long
      const origLong = longSignal;
      const origShort = shortSignal;
      longSignal = origShort;
      shortSignal = origLong;
    }

    // Bear shortOnly: block long signals, only allow shorts through
    if (inBearRegime && bearMode === 'shortOnly') {
      longSignal = false;
